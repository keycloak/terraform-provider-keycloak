...
## Example Usage (client credentials grant - private key signed JWT - key provided)
...
## Example Usage (client credentials grant - private key signed JWT - token provided)

```hcl
provider "keycloak" {
	client_id     	 = "terraform"
	client_assertion = "<private_key_signed_jwt>"
	url              = "http://localhost:8080"
}
```
...
- `client_assertion_type` - (Optional) The client assertion type used by the provider for authentication via signed JWT. Defaults to `urn:ietf:params:oauth:client-assertion-type:jwt-bearer`.
- `jwt_signing_key` - (Optional) The PEM-formatted private key used by provider to generate a signed JWT for authentication. Either `client_assertion` or `jwt_signing_key` has to be set for authentication via signed JWT.
...
- `client_assertion` - (Optional) The signed JWT used by the provider for authentication. Either `client_assertion` or `jwt_signing_key` has to be set for authentication via signed JWT.
