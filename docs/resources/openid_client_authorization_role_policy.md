---
page_title: "keycloak_openid_client_authorization_role_policy Resource"
---

# keycloak\_openid\_client\_authorization\_role\_policy Resource

Allows you to manage openid Client Authorization Role type Policies.

Role policies allow you to define conditions based on user role membership. You can specify whether all roles must be present or just one.
...
Client authorization role policies can be imported using the format: `{{realmId}}/{{resourceServerId}}/{{policyId}}`.

Example:

```bash
$ terraform import keycloak_openid_client_authorization_role_policy.test my-realm/3bd4a686-1062-4b59-97b8-e4e3f10b99da/63b3cde8-987d-4cd9-9306-1955579281d9
```
