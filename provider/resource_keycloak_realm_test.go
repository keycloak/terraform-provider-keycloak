...
func testKeycloakRealm_tokenSettings(realm string) string {
	...
	// Ensure client_session_idle_timeout <= sso_session_idle_timeout (Keycloak 25+)
	clientSessionIdleTimeout := randomDurationStringLessThanOrEqual(ssoSessionIdleTimeout)
	// Ensure client_session_max_lifespan <= sso_session_max_lifespan (Keycloak 25+)
	clientSessionMaxLifespan := randomDurationStringLessThanOrEqual(ssoSessionMaxLifespan)
