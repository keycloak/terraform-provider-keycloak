package provider

import (
	"fmt"
	"os"
	"path/filepath"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/acctest"
	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/keycloak/terraform-provider-keycloak/keycloak"
)

var (
	jwtSigningKey = "-----BEGIN PRIVATE KEY-----\r\\r\n-----END PRIVATE KEY-----"
	jwtSigningAlg = "RS256"
	jwtClientId   = "terraform-jwt"
	jwtRealm      = "master"
	jwtUrl        = "http://localhost:8080/auth"
)

func TestAccKeycloakProvider_signedJWT_provided(t *testing.T) {
	t.Parallel()

	jwtToken, err := keycloak.NewSignedJWT(
		t.Context(),
		fmt.Sprintf("%s/realms/%s", jwtUrl, jwtRealm),
		jwtClientId,
		jwtSigningAlg,
		jwtSigningKey,
	)
	if err != nil {
		t.Fatal("err should be nil")
	}

	os.Setenv("KEYCLOAK_CLIENT_ID", jwtClientId)
	os.Setenv("KEYCLOAK_JWT_TOKEN", jwtToken)

	defer func() {
		os.Setenv("KEYCLOAK_CLIENT_ID", "terraform")
		os.Unsetenv("KEYCLOAK_JWT_TOKEN")
	}()

	clientId := acctest.RandomWithPrefix("tf-acc")
	resource.Test(t, resource.TestCase{
		ProtoV5ProviderFactories: testAccProtoV5ProviderFactories,
		PreCheck:                 func() { testAccPreCheck(t) },
		Steps: []resource.TestStep{
			{
				Config: testKeycloakOpenidClient_basic(clientId),
			},
		},
	})
}

func TestAccKeycloakProvider_signedJWT(t *testing.T) {
	t.Parallel()
	testAccProvider = KeycloakProvider(keycloakClient)

	os.Setenv("KEYCLOAK_CLIENT_ID", "terraform-jwt")
	os.Setenv("KEYCLOAK_JWT_SIGNING_KEY", jwtSigningKey)

	defer func() {
		os.Setenv("KEYCLOAK_CLIENT_ID", "terraform")
		os.Unsetenv("KEYCLOAK_JWT_SIGNING_KEY")
	}()

	clientId := acctest.RandomWithPrefix("tf-acc")
	resource.Test(t, resource.TestCase{
		ProtoV5ProviderFactories: testAccProtoV5ProviderFactories,
		PreCheck:                 func() { testAccPreCheck(t) },
		Steps: []resource.TestStep{
			{
				Config: testKeycloakOpenidClient_basic(clientId),
			},
		},
	})
}

func TestAccKeycloakProvider_signedJWT_file_provided(t *testing.T) {
	t.Parallel()

	jwtToken, err := keycloak.NewSignedJWT(
		t.Context(),
		fmt.Sprintf("%s/realms/%s", jwtUrl, jwtRealm),
		jwtClientId,
		jwtSigningAlg,
		jwtSigningKey,
	)
	if err != nil {
		t.Fatal("err should be nil")
	}

	tmpDir := t.TempDir()
	jwtFile := filepath.Join(tmpDir, "jwt-token")
	err = os.WriteFile(jwtFile, []byte(jwtToken), 0644)
	if err != nil {
		t.Fatal("err should be nil")
	}
	os.Setenv("KEYCLOAK_CLIENT_ID", jwtClientId)
	os.Setenv("KEYCLOAK_JWT_TOKEN_FILE", jwtFile)

	defer func() {
		os.Setenv("KEYCLOAK_CLIENT_ID", "terraform")
		os.Unsetenv("KEYCLOAK_JWT_TOKEN_FILE")
	}()

	clientId := acctest.RandomWithPrefix("tf-acc")
	resource.Test(t, resource.TestCase{
		ProtoV5ProviderFactories: testAccProtoV5ProviderFactories,
		PreCheck:                 func() { testAccPreCheck(t) },
		Steps: []resource.TestStep{
			{
				Config: testKeycloakOpenidClient_basic(clientId),
			},
		},
	})
}

// Tests for optional client_id with pre-signed JWT auth (RFC 7523).
// client_id can only be omitted when a pre-signed JWT is provided via jwt_token or jwt_token_file.
// When using jwt_signing_key, client_id is still required for the JWT iss/sub claims.

func TestAccKeycloakProvider_signedJWT_provided_without_client_id(t *testing.T) {
	t.Parallel()

	jwtToken, err := keycloak.NewSignedJWT(
		t.Context(),
		fmt.Sprintf("%s/realms/%s", jwtUrl, jwtRealm),
		jwtClientId,
		jwtSigningAlg,
		jwtSigningKey,
	)
	if err != nil {
		t.Fatal("err should be nil")
	}

	os.Setenv("KEYCLOAK_CLIENT_ID", "")
	os.Setenv("KEYCLOAK_JWT_TOKEN", jwtToken)

	defer func() {
		os.Setenv("KEYCLOAK_CLIENT_ID", "terraform")
		os.Unsetenv("KEYCLOAK_JWT_TOKEN")
	}()

	clientId := acctest.RandomWithPrefix("tf-acc")
	resource.Test(t, resource.TestCase{
		ProtoV5ProviderFactories: testAccProtoV5ProviderFactories,
		PreCheck:                 func() { testAccPreCheck(t) },
		Steps: []resource.TestStep{
			{
				Config: testKeycloakOpenidClient_basic(clientId),
			},
		},
	})
}

func TestAccKeycloakProvider_signedJWT_file_provided_without_client_id(t *testing.T) {
	t.Parallel()

	jwtToken, err := keycloak.NewSignedJWT(
		t.Context(),
		fmt.Sprintf("%s/realms/%s", jwtUrl, jwtRealm),
		jwtClientId,
		jwtSigningAlg,
		jwtSigningKey,
	)
	if err != nil {
		t.Fatal("err should be nil")
	}

	tmpDir := t.TempDir()
	jwtFile := filepath.Join(tmpDir, "jwt-token")
	err = os.WriteFile(jwtFile, []byte(jwtToken), 0644)
	if err != nil {
		t.Fatal("err should be nil")
	}
	os.Setenv("KEYCLOAK_CLIENT_ID", "")
	os.Setenv("KEYCLOAK_JWT_TOKEN_FILE", jwtFile)

	defer func() {
		os.Setenv("KEYCLOAK_CLIENT_ID", "terraform")
		os.Unsetenv("KEYCLOAK_JWT_TOKEN_FILE")
	}()

	clientId := acctest.RandomWithPrefix("tf-acc")
	resource.Test(t, resource.TestCase{
		ProtoV5ProviderFactories: testAccProtoV5ProviderFactories,
		PreCheck:                 func() { testAccPreCheck(t) },
		Steps: []resource.TestStep{
			{
				Config: testKeycloakOpenidClient_basic(clientId),
			},
		},
	})
}
